...
class ESParameter:    
    def __init__(self, para_means, para_std=0.3, Optimizer = optim.Adam):
...
class ESModel:
    ...
    """
    def __init__(self, Model, param_std, Optimizer = optim.Adam):    
        ...
        self.param_dict = {name : ESParameter(para_means = param.clone().detach().to(device), para_std = param_std, Optimizer = Optimizer) for name, param in Model().named_parameters()}         
        self.Model = Model
