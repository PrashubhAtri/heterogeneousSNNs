...
# --------------------------
# 1. Dataset Loader (SHD)
# --------------------------
NUM_NEURONS = 700   # input channels
TIME_BINS   = 100   # fixed temporal resolution
NUM_CLASSES = 20    # SHD has 20 spoken digits


def _dense_tensor(times, units):
    """Convert sparse spike list to dense [T, N] tensor"""
    times = np.nan_to_num(times, nan=0.0, posinf=0.0, neginf=0.0)
    out = torch.zeros(TIME_BINS, NUM_NEURONS)
    if len(times):
        idx = (times / times.max() * (TIME_BINS - 1)).astype(int)
        out[idx, units] = 1.0
    return out


def load_shd():
    tr_raw = tonic.datasets.SHD(save_to="./tonic_data", train=True)
    te_raw = tonic.datasets.SHD(save_to="./tonic_data", train=False)
    def build(raw):
        xs, ys = [], []
        for ev, lb in raw:
            xs.append(_dense_tensor(ev["t"], ev["x"]))
            ys.append(lb)
        return TensorDataset(torch.stack(xs), torch.tensor(ys, dtype=torch.long))
    return build(tr_raw), build(te_raw)

TRAIN_DS, TEST_DS = load_shd()
...
def train():
    cfg = dict(nhid=200, epochs=100, batch_size=256, pop=800, std0=0.12, lr=0.01)
    wandb.init(entity='DarwinNeuron', project='EA-SHD', name="hybrid_ES_offline_low_decay", config=cfg)

    tr_loader = DataLoader(TRAIN_DS, batch_size=cfg['batch_size'], shuffle=True, num_workers=2, pin_memory=True)
